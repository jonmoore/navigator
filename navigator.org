...
*** Using D3
    ...
    The mechanism for focusing on characters, rather than using Shiny to handle state,
    uses some client-side state in the filtered nodes and links that are passed to
    displayGraph.  It is probably possible to manage that through Shiny as well, and pass
    filtered node and link data to the front-end, where D3 can merge it via the join
    handlers.  This might be a cleaner overall split of state between back-end and
    front-end.  Note that the front-end is going to manage some state anyhow, e.g. for pan
    and zoom.  Roughly, there's extra complexity in making any given update pass through
    the back-end if it could be handled front-end only, but there's also a cost to having
    closely-related state spread across front and back ends, as the reactive updates
    require much more awkward coordination than if they happen solely through SHiny or D3.
    ...
    #+begin_src js :tangle src/www/gen/d3_navigator.js
      ...
      function ensureG(parent, _class) {
          return parent
                .data([null])
                .join("g")
                .attr("class", _class);
      ...
      function filterGraphData(selectedNode, fullNodeData, fullLinkData) {
          const nodesToInclude = new Set([selectedNode]);
          fullLinkData.forEach(link => {
              if (link.source.id === selectedNode.id) {
                  nodesToInclude.add(link.target);
              } else if (link.target.id === selectedNode.id) {
                  nodesToInclude.add(link.source);
              }
          });
          return {
              filteredNodeData: [...nodesToInclude],
              filteredLinkData: fullLinkData.filter(link => {
      ...
      function linkMouseOver() {
          return function(event, d) {
              Shiny.setInputValue("focus_interaction",
      ...
      function displayGraph({svgGeom, nodeData, nodeGeom, nodeGroup, linkData, linkGroup}) {
          let forceSimulation;
          function displayGraphInner(filteredNodeData, filteredLinkData) {
              const node = nodeGroup.selectAll("g")
                    .data(filteredNodeData, d => d.id)
                    .join(enterNode(svgGeom, nodeGeom),
                          update => update,
                          exit => exit.remove());
              const link = createLink(linkGroup, filteredLinkData);
              node.on("mouseover", nodeMouseOver(link))
                  .on("mouseout", nodeMouseOut(link))
                  .on("click", (event, d) => {
                      const {filteredNodeData, filteredLinkData} = filterGraphData(d, nodeData, linkData);
                      displayGraphInner(filteredNodeData, filteredLinkData);
                  });
              link.on("mouseover", linkMouseOver());
              if (forceSimulation) {
                  forceSimulation.stop();
              }
              forceSimulation = createSimulation(svgGeom, filteredNodeData, nodeGeom, filteredLinkData);
