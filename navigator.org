...
***** D3 navigator
      ...
      #+begin_src python :tangle src/navigator/d3_core_navigator.py
        ...
        www_dir = src_dir / 'www'
        data_dir = src_dir / 'www' / 'data'

        sdf = pd.read_csv(data_dir / 'shakespeare_plays.csv')
        play_names = sorted(set(sdf['play_name']))
        print(f"jm - {sdf.head()=}")
        print(f"jm - {sorted(play_names)=}")

        sdg = shakespeare.build_shakespeare_network(sdf)
        print(f"jm - {sdg=}")
        ...
        class render_d3graph(Renderer[dict]):
            """
            Render a pandas dataframe in d3.
            """
            async def transform(self, value: dict) -> Jsonifiable:
                """
                Transform a dict representing a graph into a JSONifiable object that can be
                displayed using D3.
                """
                assert isinstance(value, dict)
                assert set(value) == {'nodes', 'links'}
        ...
        app_ui = ui.page_fluid(
            ...
            ui.div(
                ui.tags.svg(
                    id="d3-graph",
                    style="width: 1200px; height: 600px; border: 1px solid black;",
                ),
                ui.div(id="tooltip"),
                id="navigator-graph-container",
            ),
            ui.div(ui.h3("Selected Node Data"),
            ...
            def d3Graph():
                ...
                graph_data = {
                    ...
                    'links': [
                        ...
                        if edge[0][0] == play_name
                        ]
                }
                print(f"jm - {play_name=}")
                print(f"jm - {len(graph_data['nodes'])=}")
                print(f"jm - {len(graph_data['links'])=}")
                node_ids = set(n['id'] for n in graph_data['nodes'])
                edge_node_ids = set(edge['source'] for edge in graph_data['links']) | set(edge['target'] for edge in graph_data['links'])
                print(f"jm - {node_ids=}")
                print(f"jm - {edge_node_ids=}")
                print(f"jm - {node_ids-edge_node_ids=}")
                print(f"jm - {edge_node_ids-node_ids=}")
            ...
            class NavigatorGraphOutputBinding extends Shiny.OutputBinding {
                ...
                renderValue(el, payload) {
                            ...
                            enter => {
                                const nodeEnter = enter.append("g")
                                    ...
                                    .attr("transform", d => {
                                        ...
                                        const x = Math.random() * (width - nodeRadius * 2) + nodeRadius;
                                        const y = Math.random() * (height - nodeRadius * 2) + nodeRadius;
                                        return `translate(${x}, ${y})`;
                                    });
                                ...
                                nodeEnter.append("text")
                                    .attr("dy", "0.75em")
                                    .attr("y", nodeRadius + 5) // Position text below the circle
                                    .text(d => d.name)
                    ...
                    const simulation = d3.forceSimulation(node_data)
                          .force("link", d3.forceLink(link_data).id(d => d.id).distance(100))
                          .force("charge", d3.forceManyBody().strength(-300))
                          .force("center", d3.forceCenter(width / 2, height / 2))
                          .on("tick", () => update_positions(link, node, width, height));

                    // record clicks on nodes
                    node.on("click", function(event, d) {
                            // 'this' refers to the node element that the event listener is attached to
                            console.debug("node click: this:", this);
                            console.debug("node click: event:", event);
                            console.debug("node click: event.target:", event.target);
                            console.debug("node click: d:", d);
                            console.debug("node click: d.id:", d.id);
                            ...
                            // Let the server know which node was clicked
                            const clicked_node_id = d.id;
                            console.debug("clicked_node_id:", clicked_node_id);
...
***** Shakespeare data
      ...
      #+begin_src python :tangle src/navigator/shakespeare.py
        ...
        def build_shakespeare_network(df: pd.DataFrame) -> nx.Graph:
            """
            Constructs a NetworkX graph representing character interactions in Shakespeare's plays.
            ...
            Returns:
                A NetworkX Graph object with structured node and edge attributes.
            """
            required_columns = set("play_name,genre,character,act,scene,sentence,text,sex".split(','))
            assert set(df.columns) > required_columns

            # Group by play, character, and sex to get a summary for each character node.
            character_data = df.groupby(['play_name', 'character', 'sex']).agg(
                num_sentences=('sentence', 'count'),
                num_acts=('act', 'nunique')
            ).reset_index()

            # Add the number of unique scenes.
            scenes_per_character = df.groupby(['play_name', 'character', 'act', 'scene']).size().reset_index(name='_count')
            scenes_per_character = scenes_per_character.groupby(['play_name', 'character']).size().reset_index(name='num_scenes')

            character_data = character_data.merge(scenes_per_character, on=['play_name', 'character'])
            ...
            # --- 2. Process Edge Data ---
            # Find all interactions (characters speaking in the same scene).
            # Group by the scene identifier to find co-occurring characters.
            interactions = df.groupby(['play_name', 'act', 'scene'])['character'].apply(set).reset_index(name='characters_in_scene')

            edge_attributes: dict[tuple[str,str,str], dict[str, Any]] = {}

            sentences_by_scene_and_character = df.groupby(['play_name', 'act', 'scene', 'character']).size()
            for _, row in interactions.iterrows():
                ...
                for char1, char2 in combinations(set(characters_in_scene), 2):
                    ...
                    sentences_char1 = sentences_by_scene_and_character.loc[play_name, act, scene, char1]
                    sentences_char2 = sentences_by_scene_and_character.loc[play_name, act, scene, char2]

                    # If the edge doesn't exist yet, initialize it.
                    ...
                    # Append the current scene's interaction data.
                    edge_attributes[edge_key]['interactions'].append({
                        'act': act,
                        'scene': scene,
                        'sentences_char1': sentences_char1,
                        'sentences_char2': sentences_char2
                    })
        ...
        def main():
            ...
            graph = build_shakespeare_network(df)
            print(f"jm - {len(graph.edges)=}")
            print(f"jm - {len(graph.nodes)=}")
        ...
        if __name__ == '__main__':
            main()
        #     # --- Example of Conversion to JSON ---
        #     # For a simple JSON representation of the graph structure.
        #     # import json
        #     # json_data = nx.node_link_data(graph)
        #     # with open('shakespeare_graph.json', 'w') as f:
