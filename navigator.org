        ...
        if (Shiny) {
            class NavigatorGraphOutputBinding extends Shiny.OutputBinding {
                ...
                renderValue(el, payload) {
                    ...
                    // Use number of sentences to set radius
                              ...
                              enter => {
                                  const nodeEnter = enter.append("g")
                                        ...
                                        .attr("transform", d => {
                                            ...
                                            const x = Math.random() * (width - maxRadius * 2) + maxRadius;
                                            const y = Math.random() * (height - maxRadius * 2) + maxRadius;
                                            return `translate(${x}, ${y})`;
                                        });
                    ...
                    // Initially, the source and target attribute values are strings, to be
                    // matched to the node id attributes. The source and target are updated to
                    // references to corresponding node /objects/.
                    ...
                    console.debug("linkStrengthExtent", linkStrengthExtent);
                    const linkWidthScale = d3.scaleSqrt()
                          .domain(linkStrengthExtent)
                          .range([0, 10]);

                    const linkColorScale = d3.scaleSqrt()
                          .domain(linkStrengthExtent)
                          .range(["#fff", "#f00"]); // Example: from light gray to steel blue
