...
***** D3 navigator
      ...
      #+begin_src python :tangle src/navigator/d3_core_navigator.py
        ...
        PLAY_DATA = shakespeare.CharacterNetwork(pd.read_csv(www_dir / 'data' / 'shakespeare_plays.csv'))
        ...
        app_ui = ui.page_fluid(
            ...
            ui.layout_columns(
                ...
                ui.output_ui("formatted_dict_output"),
                col_widths={"xl": (8, 4)},
            ),
            ui.div(id="tooltip"),
        ...
        def server(input: shiny.Inputs, output: shiny.Outputs, _session: shiny.Session):
            ...
            def d3Graph():
                ...
                graph_data = {
                    'nodes': [
                        ...
                        for node in PLAY_DATA.graph.nodes
                        if node[0] == play_name
                    ],
                    'links': [
                        {
                        ...
                        for edge in PLAY_DATA.graph.edges
            ...
            @output
            @render.ui
            def formatted_dict_output():
                play_name = input.play_name()
                node_id = input.focus_node_id()
                html_content = "<div>"
                if node_id:
                    node_data = dict(PLAY_DATA.graph.nodes[(play_name, node_id)])
        ...
        if (Shiny) {
            class NavigatorGraphOutputBinding extends Shiny.OutputBinding {
                ...
                renderValue(el, payload) {
                    ...
                    const svg = d3.select("#d3-graph").call(d3.zoom().on("zoom", function () {
                    ...
                    node.on("mouseover", function(event, d) {
                            ...
                            // Let the server know which node has focus
                            const focus_node_id = d.id;
                            Shiny.setInputValue("focus_node_id", d.id);
      ...
      #+begin_src python :tangle src/navigator/shakespeare.py
        ...
        class CharacterNetwork:
            def __init__(self, df: pd.DataFrame):
                ...
                """
                self.df = df
                self.graph = self._build_shakespeare_network(self.df)
                self.play_names = sorted(set(df["play_name"]))

            @staticmethod
            def _build_shakespeare_network(df: pd.DataFrame) -> nx.Graph:
                required_columns = set(
                    "play_name,genre,character,act,scene,sentence,text,sex".split(",")
                )
                ...
                character_data = ((
                    ...
                    scenes_per_character, on=["play_name", "character"]
                ))
