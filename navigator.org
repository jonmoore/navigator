...
*** Using D3
    ...
    #+begin_src python :tangle src/navigator/d3/app.py :mkdirp yes
      ...
      def server(input: shiny.Inputs, output: shiny.Outputs, _session: shiny.Session):
          ...
          def create_character_calc(input_name: str):
              """Create a reactive calc that returns either the character given by input_name or
              None, if this is for a character not in the current play.

              This exists to avoid displaying stale information after we switch plays.

              Returns:
                   A reactive calc. The calc returns the character dict, with 'play' and 'name'
                   set if 1) the input has been set and 2) the character's play matches the
                   current input for 'play_name'.  Otherwise the calc returns None.
              """
              @shiny.reactive.calc
              def character_calc() -> dict | None:
                  play = input.play_name()
                  try:
                      character = input[input_name]()
                  except SilentException:
                      return None
                  if character["play"] != play:
                      return None
                  return character
              return character_calc

          # The character to zoom in the graph display for, typically chosen based on click.
          zoom_character = create_character_calc('zoom_character')
          # The character to show detail for, typically chosen based on mouseover.
          detail_character = create_character_calc('detail_character')

          @render_character_graph
          def d3Graph():
              play_name = input.play_name()
              G = PLAY_DATA.graph
              if zoom_character() is None:
                  print(f"jm - displaying all characters in {play_name}")
                  return nx.subgraph_view(G, filter_node=lambda n: n[0] == play_name)
              ac = zoom_character()
              assert ac is not None
              assert ac["play"] == play_name

              focal_node = (ac["play"], ac["name"])
              neighborhood = set([focal_node]) | set(G.neighbors(focal_node))
              print(f"jm - displaying neighbors of {ac['name']} in {play_name}")
              return nx.subgraph_view(G, filter_node=lambda n: n in neighborhood)
          ...
          def detail_character_output():
              ...
              if ac is None:
                  return ui.HTML("")
              html = character_html(
                  PLAY_DATA.graph, play=ac["play"], name=ac["name"]
              )
              return ui.HTML(html)
    ...
    #+begin_src js :tangle src/www/gen/d3_navigator.js
      ...
      function createZoomBehavior(zoomGroup){
          return d3.zoom()
              .scaleExtent([0.1, 10])
              .on("zoom", (event) => {
                  console.info("zoom detected", event);
      ...
      function nodeMouseOver(link) {
          return function(event, d) {
              Shiny.setInputValue("detail_character", {"play": d.play,
                                                      "name": d.name});
      ...
      function displayGraph({svgGeom, nodeData, nodeGeom, nodeGroup, linkData, linkGroup}) {
          ...
          function displayGraphInner(innerNodeData, innerLinkData) {
              ...
              node.on("mouseover", nodeMouseOver(link))
                  ...
                  .on("click", (event, d) => {
                      Shiny.setInputValue("zoom_character", {"play": d.play, "name": d.name});
                  });
      ...
      if (Shiny) {
          class NavigatorGraphOutputBinding extends Shiny.OutputBinding {
              ...
              renderValue(el, payload) {
                  const nodeData = payload.nodes;
                  const linkData = payload.links;
