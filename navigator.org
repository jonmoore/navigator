...
***** D3 navigator
      ...
      #+begin_src python :tangle src/navigator/d3_core_navigator.py
        ...
        d3_dep = htmltools.HTMLDependency(
            ...
            script={"src": "d3GraphComponent.js", "type": "module"},
      ...
      #+begin_src js2 :tangle src/www/gen/d3GraphComponent.js
        import * as d3 from "https://cdn.jsdelivr.net/npm/d3@7/+esm"; // Using d3 v7 ESM

        function update_positions(link, node) {
            link
        ...
        if (Shiny) {
            class NavigatorGraphOutputBinding extends Shiny.OutputBinding {
                find(scope) {
                    const function_name = "NavigatorGraphOutputBinding.find";
                    console.debug(`${function_name}, scope:`, scope);
                    const found = scope.find(".shiny-d3-graph-output");
                    console.debug(`${function_name}, found:`, found);
                    return found;
                }

                renderValue(el, payload) {
                    // el - https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement
                    const function_name = "NavigatorGraphOutputBinding.renderValue";
                    console.info(`${function_name}, payload:`, payload);
                    const node_data = payload.nodes;
                    console.info(`${function_name}, node_data:`, node_data);
                    const link_data = payload.links;
                    console.info(`${function_name}, link_data:`, link_data);
                    ...
                    console.debug(`${function_name}, svg:`, svg);
                    console.debug(`${function_name}, tooltip:`, tooltip);
                    console.debug(`${function_name}, width:`, width);
                    console.debug(`${function_name}, height:`, height);
                    ...
                    const node = svg.append("g")
                          ...
                          .enter().append("g")
                          .attr("class", "node")
                    // place the nodes at random positions
                          .attr("transform", (d, i) => {
                    ...
                    const colorTradeWind = "#69b3a2";
                    node.append("circle")
                        .attr("r", nodeRadius)
                        .attr("fill", colorTradeWind);
                    ...
                    const simulation = d3.forceSimulation(node_data)
                          ...
                          .on("tick", () => update_positions(link, node));

                    // record clicks on nodes
                    node.on("click", function(event, d) {
                            // 'this' refers to the node element that the event listener is attached to
                            console.info("node click: this:", this);
                            console.info("node click: event:", event);
                            console.info("node click: event.target:", event.target);
                            console.info("node click: d:", d);
                            console.info("node click: d.id:", d.id);
                            ...
                            console.info("clicked_node_id:", clicked_node_id);
      ...
      #+end_src
      #+NAME: d3_core_js
      #+begin_src js2 :tangle src/www/gen/d3_graph.js
        ...
        function initializeD3Graph(graph) {
            const svg = d3.select("#d3-graph");
            const tooltip = d3.select("#tooltip");
            ...
            node.on("click", function(event, d) {
                    ...
                    if (typeof Shiny !== 'undefined' && Shiny.setInputValue) {
                        ...
                        console.error("Shiny.setInputValue is not available. Ensure Shiny is loaded correctly.");
                    }
                })
                .on("mouseover", function(event, d) {
                    tooltip.transition()
        ...
        class NavigatorGraphOutputBinding extends Shiny.OutputBinding {
            find(scope) {
                return scope.find("#navigator-graph-container");
            }
