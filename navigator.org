...
******* ipysigma Shiny Core app with incremental redraw
        ...
        It can be run with
        #+begin_src bash
          shiny run --reload --log-level debug src/navigator/ipysigma_core_incremental.py
        #+end_src

        #+begin_src python :tangle src/navigator/ipysigma_core_incremental.py
          import os
          ...
          graph_id = "graph_output"
          selected_node_display_id = "selected_node_display"
          node_data_display_id = "node_data_display"

          app_ui = ui.page_fluid(
              ui.h2("Ipysigma Graph (Core API), incremental redraw"),
              shinywidgets.output_widget(graph_id),
              ui.output_text(selected_node_display_id),
              ui.output_table(node_data_display_id)
          )
          ...
          def server(_input, _output, _session: Session):
              ...
              focus_node_reactive = shiny.reactive.Value[int | None](None)

              @shinywidgets.render_widget
              def graph_output() -> Sigma:
                  focus_node = focus_node_reactive.get()
                  if focus_node is None:
                      graph = base_graph
                      extra_kwargs = {}
                  else:
                      graph = graph_utils.node_and_neighbors(base_graph, focus_node)
                      extra_kwargs = dict(selected_node=focus_node)
              ...
              def selected_node_display() -> str:
                  ...
                  focus_node_reactive.set(value=selected_node)
                  return f"The selected node is: {selected_node}"

              @shiny.render.table
              def node_data_display() -> pd.DataFrame:
                  selected_node = focus_node_reactive.get()
...
***** Conclusion
      ...
      there were 15 at the time of writing with some in quite basic areas like svg export.
